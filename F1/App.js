...
export default function App() {
  ...
  const [iosClientIdVal, setiosClientIdVal] = useState("");
  const [androidClientIdVal, setandroidClientIdVal] = useState("");
    ...
    const getExpoClientId = async () => {
      try {
        ...
        const result = await axios({
          method: "get",
          url: `http://192.168.1.71:9000/googleSignin`,
        });
        setExpoClientIdValue(result.data.expoClientId);
        setandroidClientIdVal(result.data.androidClientId);
        setiosClientIdVal(result.data.iosClientId);
      } catch (error){
        ...
        <LocationProvider>
            <NavigationContainer>
              <Stack.Navigator screenOptions={{ headerShown: false }}>
                ...
                <Stack.Screen name="Login" >{(props) => <LoginScreen {...props} expoClientIdValue={expoClientIdValue} androidClientId={androidClientIdVal} iosClientId={iosClientIdVal}/>}</Stack.Screen>
